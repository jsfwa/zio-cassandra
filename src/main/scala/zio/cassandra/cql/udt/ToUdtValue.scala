...
import zio.cassandra.cql.{Binder, CassandraTypeMapper}
import zio.cassandra.cql.udt.ToUdtValue.{make, makeWithFieldName}
...
trait LowerPriorityToUdtValue {
   ...
   * @return
   */
  implicit def deriveFromCassandraTypeMapper[A](
    implicit
...
trait LowestPriorityToUdtValue {
  ...
  implicit def hListToUdtValue[K <: Symbol, H, T <: HList](
    implicit
  ...
  implicit def genericToUdtValue[A, R](
    implicit
